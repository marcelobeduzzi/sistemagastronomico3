...
const [selectedMonth, setSelectedMonth] = useState(new Date().getMonth()) // Mes anterior por defecto (0-11)
...
const loadData = async () => {
  setIsLoading(true)
  try {
    console.log(`Cargando datos para ${displayMonth}/${selectedYear}, pestaña: ${activeTab}`)
    ...
    if (activeTab === "pendientes" || activeTab === "liquidaciones") {
      // Cargar TODAS las nóminas sin filtrar por is_paid
      console.log(`Consultando todas las nóminas`)
      const allPayrollsData = await dbService.getPayrollsByPeriod(0, 0, false)
      console.log(`Total nóminas encontradas: ${allPayrollsData.length}`)
      setAllPayrolls(allPayrollsData)
      
      // Filtrar en el cliente las nóminas pendientes
      const pendingPayrolls = allPayrollsData.filter(payroll => !payroll.isPaid)
      console.log(`Nóminas pendientes: ${pendingPayrolls.length}`)
      
      // Filtrar por mes y año si es necesario
      const filteredPayrolls = pendingPayrolls.filter(payroll => 
        payroll.month === displayMonth && payroll.year === selectedYear
      )
      console.log(`Nóminas pendientes para ${displayMonth}/${selectedYear}: ${filteredPayrolls.length}`)
      setPayrolls(filteredPayrolls)

      // Cargar liquidaciones pendientes
      const liquidationsData = await dbService.getLiquidations(false)
      console.log(`Liquidaciones pendientes encontradas: ${liquidationsData.length}`)
      setLiquidations(liquidationsData)
    } else if (activeTab === "historial") {
      // Cargar historial de nóminas pagadas
      console.log(`Consultando historial de nóminas para ${displayMonth}/${selectedYear}`)
      const historyData = await dbService.getPayrollsByPeriod(displayMonth, selectedYear, true)
      console.log(`Nóminas pagadas encontradas: ${historyData.length}`)
...
const handleGeneratePayrolls = async () => {
  ...
  try {
    console.log(`Generando nóminas para ${displayMonth}/${selectedYear}`)
    // Llamar al servicio para generar nóminas
    await dbService.generatePayrolls(displayMonth, selectedYear)
...
// Función para mostrar todas las nóminas pendientes
const showAllPendingPayrolls = () => {
  // Filtrar solo las nóminas no pagadas
  const pendingPayrolls = allPayrolls.filter(payroll => !payroll.isPaid)
    ...
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        {activeTab === "pendientes" && (
          <Button variant="secondary" onClick={showAllPendingPayrolls}>
      ...
      <Tabs defaultValue="pendientes" value={activeTab} onValueChange={setActiveTab}>
            ...
            <CardContent>
              {payrolls.length === 0 && !isLoading ? (
                <div className="text-center py-8">
                  <p className="text-muted-foreground mb-4">No hay nóminas pendientes para este período</p>
                  <div className="flex justify-center space-x-4">
                    <Button onClick={handleGeneratePayrolls} disabled={isGeneratingPayrolls}>
                      <RefreshCw className={`mr-2 h-4 w-4 ${isGeneratingPayrolls ? "animate-spin" : ""}`} />
                      Generar Nóminas
                    </Button>
                    <Button variant="outline" onClick={showAllPendingPayrolls}>
                      Ver Todas las Pendientes
                    </Button>
                  </div>
                </div>
        ...
        <TabsContent value="historial">
          <Card>
            ...
            <CardContent>
              {historyPayrolls.length === 0 && !isLoading ? (
                <div className="text-center py-8">
                  <p className="text-muted-foreground">No hay nóminas pagadas para este período</p>
                </div>
              ) : (
                <DataTable
